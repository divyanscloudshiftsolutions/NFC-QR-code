  ...
  return (
    <div className="space-y-6 text-text-main animate-fadeIn">
          ...
          {priorityActionsList.map((act, i) => {
            ...
            return (
              <button
                key={i}
                onClick={act.onClick}
                className={`w-full p-4 rounded-xl flex items-center justify-between transition-all duration-300 text-left cursor-pointer group focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/50 ${
                  act.primary
                    ? 'bg-gradient-to-br from-[#D4AF37]/10 to-transparent border border-[#D4AF37]/30 hover:border-[#D4AF37] shadow-[0_0_12px_rgba(212,175,55,0.08)]'
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h5 className={`text-xs font-bold transition-colors ${act.primary ? 'text-white' : 'text-text-main group-hover:text-white'}`}>
                      {act.title}
                    </h5>
                    <p className="text-[10px] text-text-muted mt-0.5">{act.desc}</p>
                  </div>
                </div>
                <div className={`text-xs font-bold transition-transform group-hover:translate-x-1 ${act.primary ? 'text-[#D4AF37]' : 'text-text-muted group-hover:text-text-main'}`}>
                  →
                </div>
              </button>
            );
      ...
      <div className="space-y-3">
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 ${isManagement ? 'lg:grid-cols-5' : 'lg:grid-cols-3'} gap-4`}>
          {metricCards.map((card, i) => {
            const Icon = card.icon;
            return (
              <div key={i} className={`glass-panel p-4 rounded-xl flex items-center justify-between border-l-4 ${card.colorClass} border-border-main/50 gap-4`}>
                <div className="min-w-0">
                  <p className="text-[9px] font-bold text-text-muted uppercase tracking-wider truncate">{card.title}</p>
                  <h3 className="text-xl sm:text-2xl font-black text-text-main mt-1 tracking-tight">{card.value}</h3>
                  <p className="text-[10px] mt-1 font-semibold truncate flex items-center gap-1">{card.sub}</p>
                </div>
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${card.iconBgClass}`}>
                  <Icon size={20} />
                </div>
              </div>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 glass-panel rounded-2xl p-4 sm:p-6 border border-border-main space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="py-12 text-center text-text-muted text-xs select-none space-y-3">
        ...
        <div className="glass-panel p-4 sm:p-6 rounded-2xl border border-border-main flex flex-col justify-between min-h-[300px] lg:h-[396px]">
          ...
          <div className="space-y-3 flex-1 overflow-y-auto my-3 pr-1 custom-scrollbar">
            {notifications.length === 0 ? (
              <div className="flex-1 flex items-center justify-center h-full text-text-muted text-xs select-none">
                  ...
                  <div key={notif.id} className="p-3 rounded-xl bg-bg-secondary-surface dark:bg-black/10 border border-border-main flex flex-col sm:flex-row items-start sm:items-center gap-3 justify-between animate-fadeIn">
                    <div className="flex items-start gap-3 min-w-0 flex-1">
                      ...
                      <div className="flex-1 min-w-0 text-left">
                        ...
                        <p className="text-[11px] text-text-muted mt-1 font-semibold">{notif.message}</p>
                    ...
                    {notif.actionLabel && notif.onAction && (
                      <button
                        ...
                        className="w-full sm:w-auto px-2.5 py-1 rounded bg-[#D4AF37]/10 hover:bg-[#D4AF37]/25 text-[#D4AF37] text-[10px] font-bold border border-[#D4AF37]/20 transition-all cursor-pointer whitespace-nowrap self-stretch sm:self-center flex items-center justify-center shrink-0"
                      >
                        {notif.actionLabel}
                      </button>
                    )}
            ...
            <div className="glass-panel p-4 sm:p-6 rounded-2xl border border-border-main flex flex-col justify-between min-h-[340px]">
              <div className="flex items-center justify-between pb-3 border-b border-border-main select-none shrink-0">
                ...
                <span className="text-[10px] font-bold dark:text-emerald-400 text-emerald-700 flex items-center gap-1 shrink-0">
                  <TrendingUp size={12} /> {peakSalesHourStr}
                </span>
              </div>
                    ...
                    <div className="flex gap-2 items-stretch h-36">
                      ...
                      <div className="flex-1 border-l border-b border-border-main px-2 flex justify-between items-end relative h-full">
                        ...
                        {revenueTrends.map((trend, idx) => {
                          ...
                          return (
                            <div key={idx} className="flex flex-col items-center flex-1 group h-full justify-end relative z-10 mx-0.5">
                ...
                {seatingPeaksTrends.length === 0 ? (
                  ...
                  <div className="min-w-[280px]">
                    <div className="relative w-full h-[144px] flex items-end">
                      <svg className="w-full h-full" viewBox="0 0 500 144" preserveAspectRatio="none">
                        <defs>
